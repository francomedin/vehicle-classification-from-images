...
def predict_from_folder(folder, model, input_size, class_names):
    ...
    for dirpath, filename in walkdir(folder):
        img = tf.keras.utils.load_img(
            os.path.join(dirpath, filename), target_size=input_size
        )
        img_array = tf.keras.utils.img_to_array(img)
        change_img_size = smart_resize(img_array, input_size)
        # I was getting and input shape problem without reshaping.
        change_img_size = change_img_size.reshape([1, 224, 224, 3])
        model_predictions = model.predict(change_img_size)
        best_prediction = model_predictions.argmax(axis=1)
        predicted_class = class_names[best_prediction[0]]
